...
SELECT m.name, m.brand_name, b.headquarters FROM models as m, brands as b WHERE b.name=m.brand_name AND m.name='Mustang';
...
SELECT * FROM models ORDER BY name OFFSET 14 ROWS LIMIT 10;
...
SELECT m.brand_name FROM models m GROUP BY m.brand_name HAVING COUNT(m.brand_name) > 5;
...
SELECT m.name, m.year FROM models m, brands b WHERE m.year = b.founded;